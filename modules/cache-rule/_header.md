...
Cache rules that pull from a rate-limited or authenticated upstream (such as Docker Hub) reference a **credential set** via `credential_set_resource_id`. Deploy the credential set with the sibling `credential-set` submodule and pass its `resource_id` in. Public upstreams like Microsoft Container Registry (MCR) do not require a credential set.
